...
import { StyledNumber, StyledRow } from "./HospitalRow";
...
const StyledHeading = styled(StyledNumber)`
  ...
  background-color: #f8f8f8;
  ${({ sortable }) =>
    sortable ? "cursor: pointer; &:hover {background-color: #eee;}" : null}
...
const StyledOrderIcon = styled(FilterListIcon)`
  ...
  transform: ${({ descending }) =>
    !descending ? "rotate(180deg)" : "rotate(0)"};
...
function HospitalGridHeader(props) {
  ...
  function getHandleClick(bedType) {
    return (event) => {
      if (bedType == sortValue.field) {
        setSortValue({
          ...sortValue,
  ...
  function renderHeading(index, key, text, active, descending) {
    return (
      <StyledHeading
        ...
        key={index}
        onClick={getHandleClick(key)}
        sortable
      >
        {text}
        <StyledOrderIcon
          active={active ? 1 : 0}
          descending={active ? (descending ? 1 : 0) : 1}
        />
      </StyledHeading>
    );
  }
  ...
  return (
    <StyledRow visible>
      ...
      >
        ...
        <StyledOrderIcon
          active={sortValue.field == "distance" ? 1 : 0}
          descending={
            sortValue.field == "distance" ? (sortValue.descending ? 1 : 0) : 1
          }
        />
      </StyledHeading>
