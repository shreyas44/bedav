...
function FilterScreen(props) {
  ...
  const [categories, mobileCategories] = [
    useCategories(),
    useMobileCategories(),
  ];
  ...
  let CategoryFilterFields = Object.keys(categories).map((key) => (
    <FilterField key={key} value={categoriesReverseMap[key]}>
      {categories[key]} ({mobileCategories[categories[key]]})
    </FilterField>
  ));
