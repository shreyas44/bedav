...
def get_data(driver, city):
    table = driver.find_element_by_id(
        "dataTable").find_element_by_tag_name("tbody")
    rows = table.find_elements_by_tag_name("tr")

    data = pd.DataFrame(columns=["name", "gen_total", "oxy_total", "ICU_total", "vent_total",
                                 "gen_occupied", "oxy_occupied", "ICU_occupied", "vent_occupied", "city"])
    ...
    data[["gen_total", "oxy_total", "ICU_total", "vent_total", "gen_occupied", "oxy_occupied", "ICU_occupied", "vent_occupied"]] = data[["gen_total", "oxy_total", "ICU_total", "vent_total", "gen_occupied",
                                                                                                                                         "oxy_occupied", "ICU_occupied", "vent_occupied"]].applymap(lambda value: re.sub(',', '', str(value))).apply(pd.to_numeric, errors="coerce").applymap(lambda value: value if value > 0 else -1 * value)
    ...
    for index, item in data[["name", "city"]].iterrows():
        ...
        def hospital_exists(name):
            obj = Hospital.objects.filter(
                name=name, locality_id=locality_id).first()
...
def update_data(data, locality_id):
    ...
    for index, item in data.iterrows():
        ...
        hospital = Hospital.objects.filter(
            name=item["name"], locality_id=locality_id).first()
        ...
        if item["oxy_occupied"] > item["oxy_total"]:
            del available["oxy"]
...
def get_all():
    ...
    for i in range(0, len(ids)):
        ...
        home_button = driver.find_element_by_class_name(
